...
#        GameState.explored.add(self)
    ...
    def generate_successor(self, agent_index, action):
        ...
        if agent_index == 0:  # Pacman is moving
            state.data._eaten = [False for i in range(state.num_agents())]
...
class PacmanRules:
    ...
    def apply_action(state, action):
        ...
        pacmanState.configuration = pacmanState.configuration.generate_successor(
            vector)
    ...
    def consume(position, state):
        ...
        if(position in state.get_capsules()):
            ...
            for index in range(1, len(state.data.agentStates)):
                state.data.agentStates[index].scared_timer = SCARED_TIME
...
class GhostRules:
    ...
    def apply_action(state, action, ghostIndex):
    ...
    def decrementTimer(ghost_state):
    ...
    def checkDeath(state, agent_index):
        ...
        if agent_index == 0:  # Pacman just moved; Anyone can kill him
            for index in range(1, len(state.data.agentStates)):
                ...
                ghostPosition = ghost_state.configuration.get_position()
    ...
    def placeGhost(state, ghost_state):

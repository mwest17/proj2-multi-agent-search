...
#     state.get_score()
#           used by multiAgents.score_evaluation_function, which is the default
#
import testClasses
import json
...
class GradingAgent(Agent):
    ...
    def get_action(self, state):
        ...
        studentAction = (self.studentAgent.get_action(state),
...
class GraphGameTreeTest(testClasses.TestCase):
    ...
    def solveProblem(self, multiAgents):
        self.problem.reset()
        studentAgent = getattr(multiAgents, self.alg)(depth=self.depth)
        action = studentAgent.get_action(self.problem.startState)
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        stats = {'time': totalTime, 'wins': [g.state.is_win() for g in games].count(True),

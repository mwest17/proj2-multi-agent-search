...
class GameStateData:
    ...
    def _foodWallStr(self, has_food, has_wall):
        if has_food:
            return '.'
        elif has_wall:
            return '%'
                ...
                if self.catchExceptions:
                    try:
                        timed_func = TimeoutFunction(
                            agent.register_initial_state, int(self.rules.getMaxStartupTime(i)))
                        try:
                ...
                if self.catchExceptions:
                    try:
                        timed_func = TimeoutFunction(agent.observationFunction, int(
                            self.rules.getMoveTimeout(agent_index)))
                        try:
                            start_time = time.time()
                        ...
                        self._agentCrash(agent_index, quiet=False)
                        self.unmute()
                        return
                else:
            ...
            if self.catchExceptions:
                try:
                    timed_func = TimeoutFunction(agent.get_action, int(
                        self.rules.getMoveTimeout(agent_index)) - int(move_time))
                    try:
                        start_time = time.time()
                        if skip_action:
                        ...
                        self.agentTimeout = True
                        self._agentCrash(agent_index, quiet=True)
                        self.unmute()
                        return
                    ...
                    if move_time > self.rules.getMoveWarningTime(agent_index):
                        self.totalAgentTimeWarnings[agent_index] += 1
                        print("Agent %d took too long to make a move! This is warning %d" % (
                            agent_index, self.totalAgentTimeWarnings[agent_index]), file=sys.stderr)
                        if self.totalAgentTimeWarnings[agent_index] > self.rules.getMaxTimeWarnings(agent_index):
                    ...
                    self.totalAgentTimes[agent_index] += move_time
                    # print "Agent: %d, time: %f, total: %f" % (agent_index, move_time, self.totalAgentTimes[agent_index])
                    if self.totalAgentTimes[agent_index] > self.rules.getMaxTotalTime(agent_index):
                        print("Agent %d ran out of time! (time: %1.2f)" % (
                            agent_index, self.totalAgentTimes[agent_index]), file=sys.stderr)
                        self.agentTimeout = True
                        self._agentCrash(agent_index, quiet=True)
                        self.unmute()
                        return

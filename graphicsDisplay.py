...
class PacmanGraphics:
    ...
    def swapImages(self, agent_index, newState):
        """
        ...
        if newState.isPacman:
            image = self.drawPacman(newState, agent_index)
            self.agentImages[agent_index] = (newState, image)
        else:
            image = self.drawGhost(newState, agent_index)
            self.agentImages[agent_index] = (newState, image)
        refresh()

    def update(self, newState):
        agent_index = newState._agentMoved
        agentState = newState.agentStates[agent_index]

        if self.agentImages[agent_index][0].isPacman != agentState.isPacman:
            self.swapImages(agent_index, agentState)
        prevState, prevImage = self.agentImages[agent_index]
        if agentState.isPacman:
            self.animatePacman(agentState, prevState, prevImage)
        else:
            self.moveGhost(agentState, agent_index, prevState, prevImage)
    ...
    def animatePacman(self, pacman, prevPacman, image):
        ...
        if self.frameTime > 0.01 or self.frameTime < 0:
            start = time.time()
            fx, fy = self.get_position(prevPacman)
            ...
            self.movePacman(self.get_position(pacman),
    ...
    def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
        old_x, old_y = self.to_screen(self.get_position(prevGhost))
        ...
        self.moveEyes(self.get_position(ghost),
                      self.getDirection(ghost), ghostImageParts[-4:])
        refresh()

    def get_position(self, agentState):
...
class FirstPersonPacmanGraphics(PacmanGraphics):
    ...
    def get_position(self, ghost_state):
        if not self.showGhosts and not ghost_state.isPacman and ghost_state.get_position()[1] > 1:
            return (-1000, -1000)
        else:
